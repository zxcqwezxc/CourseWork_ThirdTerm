				...
				OOP.forArr(objs, function (el2, idEl2) {
					...
					if (el2.plType == 2) { // wall
						...
						if (el.isStaticIntersect(box2)) {
							...
							if (box1.x+box1.w > box2.x+box2.w/5 && box1.x < box2.x+box2.w-box2.w/5) {
								if (el.speed.y > 0 && box1.y+box1.h < box2.y+box2.h/2) {
									el.y = -box1.h-el.box.y+box2.y;
									el.speed.y = 0;
									el.jumped = false;
								} else if (el.speed.y < 0 && box1.y > box2.y+box2.h-box2.h/2) {
									el.speed.y *= -0.5;
									el.jumped = true;
								}
							}

							if (box1.y+box1.h > box2.y+box2.h/5 && box1.y < box2.y+box2.h-box2.h/5) {
								if (el.speed.x > 0 && box1.x+box1.w < box2.x+box2.w/2) {
									el.x = box2.x-box1.w-el.box.x+1;
									if (el.plType != 11) el.speed.x = 0;
									else el.speed.x = -el.speed.x;
								} else if (el.speed.x < 0 && box1.x > box2.x+box2.w-box2.w/2) {
